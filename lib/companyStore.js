...
export function setCandidateStage(id, stage, reason) {
  const store = ensureSeeded();
  store.candidates = store.candidates.map((c) => (c.id === id ? applyStage(c, stage, reason) : c));
  return write(store).candidates;
}
export function bulkSetCandidateStage(ids, stage, reason) {
  const store = ensureSeeded();
...
function currentTemplates(store) {
  return (store.settings.pipeline && store.settings.pipeline.rejectionTemplates) || [];
}
...
// ---- data retention (demo purge) --------------------------------

export function purgeRejectedCandidates(months) {
  const store = ensureSeeded();
...
export function verifyCompanyEmail() {
  const store = getStore();
